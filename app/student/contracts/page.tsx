...
function Seal({ label, type }: { label: string; type: "success" | "gold" }) {
  ...
  return (
    <div className="inline-flex flex-col items-center -rotate-6 select-none animate-pulse" style={{ animationDuration: '3s' }}>
      <svg width="76" height="72" viewBox="0 0 68 68" fill="none">
        <circle cx="34" cy="34" r="31" stroke={color} strokeWidth="1.5" />
        <circle cx="34" cy="34" r="26" stroke={color} strokeWidth="1" strokeDasharray="3 3" />
        <path d="M22 34.5L30.5 43L46 25" stroke={color} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
...
function Field({ caption, value, highlight = false }: { caption: string; value: React.ReactNode; highlight?: boolean }) {
  return (
    <div
      className={`rounded-xl px-4 py-3.5 border transition-all duration-200 hover:border-slate-300 ${
        highlight
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h4 className={`${display.className} text-slate-900 border-b border-slate-100 pb-2 text-[16px] font-bold tracking-tight mt-8 mb-4 flex items-center gap-2`}>
...
export default function StudentContractPage() {
  ...
  return (
    <div className={`${body.className} w-full min-h-screen bg-slate-50/40 py-8 px-4 print:p-0 print:bg-white flex flex-col items-center`}>
      ...
      {/* KHỐI CONTAINER ĐỂ CĂN GIỮA TOÀN BỘ NỘI DUNG */}
      <div className="w-full max-w-[760px] flex flex-col">
        ...
        {contract ? (
          <div className="space-y-6 w-full">
            {/* TỜ GIẤY HỢP ĐỒNG CHÍNH ĐẶT Ở CHÍNH GIỮA */}
            <div className="relative bg-white rounded-3xl border border-slate-200 shadow-[0_25px_60px_-15px_rgba(13,27,42,0.08)] overflow-hidden print:border-none print:shadow-none w-full">
              ...
              <div className="p-6 sm:p-10 md:p-12 space-y-8">
                ...
                <div className="text-center space-y-1.5">
                  ...
                  <p className="text-xs font-bold uppercase text-slate-700 tracking-widest border-b border-slate-900 inline-block pb-1 px-4">
                  ...
                  <div className="pt-6">
                    ...
                    <div className="flex justify-center gap-3 mt-4 text-[11px] font-bold">
            ...
            {/* KHỐI CÁC NÚT BUTTON TO RÕ, ĐẸP MẮT (ẨN KHI IN PHÔI) */}
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.02)] flex flex-col sm:flex-row gap-4 print:hidden w-full">
              <button
                ...
                className="flex-1 py-3.5 px-6 flex items-center justify-center gap-2.5 bg-slate-900 text-amber-400 rounded-xl font-bold text-[14px] hover:bg-slate-800 transition-all duration-200 active:scale-[0.99] shadow-lg shadow-slate-900/10"
              >
              ...
              {contract.status === "ACTIVE" && (
                <div className="flex flex-1 gap-3">
                  <button
                    ...
                    className="flex-1 py-3.5 px-5 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-[14px] transition-all duration-200 active:scale-[0.99] shadow-md shadow-blue-600/10"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 py-3.5 px-5 flex items-center justify-center gap-2 bg-rose-50 hover:bg-red-50 text-rose-600 border border-rose-200 font-bold rounded-xl text-[14px] transition-all duration-200 active:scale-[0.99]"
                  >
                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-16 text-center shadow-sm max-w-xl mx-auto mt-12 w-full">
            ...
            <h2 className="text-xl font-bold text-slate-900 mb-2">Chưa ghi nhận hợp đồng điện tử</h2>
